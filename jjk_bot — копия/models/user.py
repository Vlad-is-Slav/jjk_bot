...
class User(Base):
    ...
    # Текущая колода (4 слота)
    slot_1_card_id = Column(Integer, nullable=True)  # Главный персонаж
    slot_2_card_id = Column(Integer, nullable=True)  # Оружие/Шикигами
    slot_3_card_id = Column(Integer, nullable=True)  # Оружие/Шикигами/Пакт
    slot_4_card_id = Column(Integer, nullable=True)  # Врожденная техника
    ...
    def add_experience(self, amount: int):
        ...
        while self.experience >= self.experience_to_next:
            ...
            self.level += 1
            self.experience_to_next = int(self.experience_to_next * 1.2) + 50
            self.points += 10  # Бонус очков за уровень
            self.coins += 100  # Бонус монет за уровень
            leveled_up = True
